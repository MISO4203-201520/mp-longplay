...
public class PurchaseServiceTest {
    ...
    public static List<PurchaseDTO> data = new ArrayList<>();

    private static PurchaseDTO createTestObject() {
    ...
    public static void insertData() {
        ...
        for (int i = 0; i < 5; i++) {
            PurchaseDTO dto = createTestObject();
            data.add(dto);
            System.out.println("data " + dto.getCardNumber());
        }
    }
    ...
    public void createPurchaseTest() throws IOException {
        try {
            ...
            if (auth != null) {
                ...
                PurchaseDTO result = (PurchaseDTO) response.readEntity(PurchaseDTO.class);

                Assert.assertEquals(dto.getCardNumber(), result.getCardNumber());
    ...
    @Test
    @RunAsClient
    public void getPurchasesTest() throws IOException {
        try {
            Cookie auth = getAuthorization();
            if (auth != null) {
                ...
                System.out.println(response.toString());
                List<PurchaseDTO> list = new ObjectMapper().readValue(response.readEntity(String.class), List.class);
    ...
    public void getPurchaseTest() {
        try {
            ...
            if (auth != null) {
                ...
                Client cliente = ClientBuilder.newClient();
                PurchaseDTO dto = cliente.target(URLBASE + PATH).path("/" + data.get(0).getId()).request().cookie(auth).get(PurchaseDTO.class);

                Assert.assertEquals(dto.getCardNumber(), data.get(0).getCardNumber());
            } else {
    ...
    public void updatePurchaseTest() throws IOException {
        try {
            ...
            if (auth != null) {
                PurchaseDTO dto = data.get(0);
                PurchaseDTO changed = createTestObject();
                dto.setCardNumber(changed.getCardNumber());
                Client cliente = ClientBuilder.newClient();
                ...
                PurchaseDTO test = (PurchaseDTO) response.readEntity(PurchaseDTO.class);

                Assert.assertEquals(Ok, response.getStatus());
                Assert.assertEquals(dto.getCardNumber(), test.getCardNumber());
            } else {
